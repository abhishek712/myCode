...
public:
	...
	vector<int> g;
	bool check(LL m, vector<pair<int,LL> > e){
		int pos=0;
		REP(i,g.size()){
			LL temp=m;
			while(temp!=0){
...
long long SpaceWarDiv1::minimalFatigue(vector <int> girl, vector <int> es, vector<long long> ec) {
	g=girl;sort(g.begin(),g.end(),greater<int>());
	vector<pair<int,LL> > e;
	REP(i,es.size()){
		e.push_back(make_pair(es[i],ec[i]));
	}
	sort(e.begin(),e.end(),greater<pair<int,LL> >());
	LL min=1,max=5e15+1LL;
	while(min!=max){
		LL i=(min+max)/2;
		if(check(i,e))max=i;
		else min=i+1;
	}
	if(min==5e15+1LL)return -1;
	return min;
}
