...
double EllysRoomAssignmentsDiv2::getProbability(vector <string> rat) {
	int r[65500];
	int num=0;
	set<int> s;
	int temp=0;
	REP(i,rat.size()){
		...
		REP(j,rat[i].size()){
			if(rat[i][j]!=' ')temp=temp*10+rat[i][j]-'0';
			else {r[num++]=temp;temp=0;
			//cout<<r[num-1]<<" ";
			}
		}
		
	}
	if(rat[rat.size()-1][rat[rat.size()-1].size()-1]!=' '){r[num++]=temp;
		//cout<<r[num-1]<<" ";
		}
	REP(i,num)cout<<r[i]<<" ";
	int me=r[0];
	REP(i,num)s.insert(r[i]);
	num=s.size();
	int n,i=0;
	...
	int R;
	if(num%20==0)R=num/20;
	else R=num/20+1;
	//cout<<n<<endl;cout<<num<<endl;
	if(n==1)return 1.0;
	if(n<=R)return 0.0;
	//if(n<=(R-1)*20)return 1.0/R;
	return 1.0/R;
}
