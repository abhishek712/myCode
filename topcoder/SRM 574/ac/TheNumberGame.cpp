...
public:
	...
	bool solve(int a,int b, int num,int flip){
		//cout<<a<<" "<<b<<" "<<num<<" "<<flip<<endl;
		if(num==0)return 0;
		//if(a<1e7&&win[a])return 1;
		if(a==0&&b!=0)return 0;
		bool ans=0;
		if(a==b){ans=1;goto done;}
		if(solve(a/10,b,num-1,0)){ans=1;goto done;}
		if(flip==0){
		int x=1,y=1,c=0;
		while(x<a)x*=10;
		x/=10;
		...
		if(solve(c,b,num-1,1)){ans=1;goto done;}}
		done:;
		//if(a<1e7)win[a]=ans;
		return ans;
	}
...
string TheNumberGame::determineOutcome(int A, int B) {
	//CLR(win);
	if(solve(A,B, 500,0))return "Manao wins";
	return "Manao loses";
}
