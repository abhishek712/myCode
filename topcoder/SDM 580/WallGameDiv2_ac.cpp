...
int WallGameDiv2::play(vector <string> c) {
	int row=c.size(),col=c[1].size();
	int res[51][51];
	REP(i,row)
		REP(j,col)
			res[i][j]=(int)-1e9;
	...
	FOR(i,1,row-2){
		REP(j,col){
			VI temp(col,(int)-1e9);
			if(c[i][j]!='x'){
				temp[j]=res[i-1][j]+c[i][j]-'0';
				for(int k=j-1;k>=0;k--){
					if(c[i][k]!='x')
					temp[k]=temp[k+1]+c[i][k]-'0';
					else break;
				}
			...
			REP(k,col)
				res[i][k]=max(res[i][k],temp[k]);
		}
	}		
	int ans=0;
	REP(i,col)
		if(c[row-1][i]!='x'&&res[row-2][i]+c[row-1][i]-'0'>ans)ans=res[row-2][i]+c[row-1][i]-'0';
	return ans;
}
