...
int Egalitarianism::maxDifference(vector <string> edge, int d) {
	int num=edge.size();
	bool visited[55];
	int n=0;
	...
	REP(j,num){
		...
		while(!q.empty()){
			...
			m=max(m,a.second);
			REP(i,num){
				if(a.first!=i)if(edge[a.first][i]=='Y'&&!visited[i])q.push(make_pair(i,a.second+1));
			}	
		}
		int ans=0;
			REP(i,num)if(visited[i])ans++;
			if(ans!=num)return -1;
		n=max(n,m);
	}
