...
public:
	...
	bool isPrefix(string s1,string s2){//if s1 is prefix of s2
		bool ans=1;
...
int UndoHistory::minPresses(vector <string> lines) {
	set<string> hist;
	string textBuffer="";
	hist.insert(textBuffer);
	int res=0;
	REP(i,lines.size()){
		string s=lines[i];
		int j;
		for(j=0;j<s.size();j++){
			if(hist.count(s.substr(0,s.size()-j))>0){
				break;
			}
		}
		j=s.size()-j;
		int j1=0;
		if(isPrefix(textBuffer,s)){
			//cout<<textBuffer<<" "<<s<<endl;
			j1=textBuffer.size();
			if(j>j1+2){
				res+=2;
			}
